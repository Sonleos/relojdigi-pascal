...
# ==========================================
# Función para monitorizar las alarmas programadas localmente
...
# Función para mostrar la alerta emergente tanto para alarmas locales como para alertas recibidas desde el hardware. 
# El mensaje se adapta según el origen de la alerta, y se asegura de que solo una alerta esté abierta a la vez.
def mostrar_alerta_emergente(origen_local=False):
    ...
    ventana_alerta.title("¡ALARMA PROGRAMADA ACTIVADA!")
    ...
    frame_interior = ctk.CTkFrame(ventana_alerta, border_width=2, border_color="#DDE00E")
    frame_interior.pack(fill="both", expand=True, padx=10, pady=10)

    ctk.CTkLabel(frame_interior, text="🔔 ALARMA ACTIVADA ", font=("Consolas", 24, "bold"), text_color="#33FF5F").pack(pady=(20, 5))
    
    mensaje_detalle = "La alerta se esta emitiendo\nVisualizando por la PC." if origen_local else "Está reproduciendo la alarma\nen sus paneles."
    ...
    ctk.CTkButton(frame_interior, text="De acuerdo", fg_color="#1BC015", hover_color="#359E15FF",
...
# ==========================================
# Actualiza la lista de alarmas en la interfaz gráfica
...
# La función bitRead se utiliza para verificar si un bit específico está activo en un número entero.
# En este caso, se emplea para determinar qué días de la semana están seleccionados para una alarma, ya que cada día se representa como un bit en una máscara de 7 bits.
def bitRead(valor, bit):
    return (valor >> bit) & 0x01
# La función boton_eliminar_alarma se encarga de eliminar una alarma específica de la lista de alarmas en el estado.
# Recibe el id_alarma como argumento, filtra la lista para excluir la alarma con ese ID, y luego actualiza la interfaz visual para reflejar el cambio. Además, registra un mensaje en el log indicando que la alarma ha sido eliminada.
...
# La función switch_activar_alarma se encarga de cambiar el estado de activación de una alarma específica.
# Recibe el id_alarma y la variable del switch (var_estado) como argumentos, actualiza el estado de la alarma correspondiente en la lista de alarmas, y registra un mensaje en el log indicando el nuevo estado de la alarma.
...
# ==========================================
# La función buscar_puertos se encarga de detectar los puertos COM disponibles y actualizar el combo box.
# Se ejecuta automáticamente cada 2 segundos si no hay una conexión activa, pero también puede ser forzada manualmente con el botón "Recargar".
# Cuando se fuerza la búsqueda, se muestra un mensaje con los resultados encontrados.
...
# La función buscar_puertos se ejecutará cada 2 segundos automáticamente para mantener la lista actualizada, pero solo si no hay una conexión activa.
# El botón "Recargar" permite forzar una actualización inmediata y muestra un mensaje con los resultados.
def tarea_conectar():
...
# La función conectar se encarga de iniciar el proceso de conexión al puerto seleccionado.
# Si no hay una conexión activa, se lanza un hilo para ejecutar la tarea de conexión sin bloquear la interfaz gráfica.
def conectar():
...
# La función sincronizar_automatico obtiene la hora actual del sistema, la formatea en el formato esperado por el hardware (con hora en formato 12 horas, minutos, segundos, indicador AM/PM y día de la semana), y envía este comando al Arduino para sincronizar su reloj interno.

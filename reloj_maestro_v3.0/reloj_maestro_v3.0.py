...
class AppState:
    def __init__(self):
        ...
        self.MAX_ALARMAS_HARDWARE = 7  # El límite físico establecido en el Arduino
...
def toggle_log():
    if estado.log_visible:
        panel_bitacora.pack_forget()
        btn_toggle_log.configure(text="👁️ Mostrar Terminal", fg_color="#2b2b2b", hover_color="#333333")
        estado.log_visible = False
    else:
        panel_bitacora.pack(side="right", fill="both", padx=(10, 0))
        btn_toggle_log.configure(text="👁️ Ocultar Terminal", fg_color="#444444", hover_color="#555555")
...
# ==========================================
# MONITOR LOCAL Y VENTANA EMERGENTE
# ==========================================
...
def mostrar_alerta_emergente(origen_local=False):
    ...
    ventana_alerta.title("¡PROGRAMACION ACTIVADA!")
    ...
    frame_interior = ctk.CTkFrame(ventana_alerta, border_width=2, border_color="#0B7DE7")
    frame_interior.pack(fill="both", expand=True, padx=10, pady=10)

    ctk.CTkLabel(frame_interior, text="🔔 ALARMA ACTIVADA", font=("Consolas", 24, "bold"), text_color="#FFFFFF").pack(pady=(20, 5))
    
    mensaje_detalle = "La alerta se está emitiendo de forma local\nPor favor verificar conexión." if origen_local else "El reloj/Arduino está reproduciendo\nla alarma desde el equipo."
    ...
    ctk.CTkButton(frame_interior, text="De acuerdo", fg_color="#097905", hover_color="#144B04",
                command=cerrar_alerta).pack(pady=(10, 15))

# ==========================================
# LÓGICA DE ACTUALIZACIÓN DE LA LISTA VISUAL
# ==========================================
def actualizar_lista_visual():
    ...
    for al in estado.alarmas:
        ...
        ctk.CTkLabel(fila, text=f"{int(al['hora']):02d}:{int(al['min']):02d}", font=("Consolas", 22, "bold"), text_color="#ffffff").pack(side="left", padx=5)
...
def boton_eliminar_alarma(id_alarma):
    enviar(f"A:OFF:{id_alarma}")
    estado.alarmas = [a for a in estado.alarmas if a["id"] != id_alarma]
    registrar_log(f"Alarma de ranura ID {id_alarma} eliminada del sistema.")
...
def switch_activar_alarma(id_alarma, var_estado):
    ...
    for al in estado.alarmas:
        if al["id"] == id_alarma:
            ...
            cmd = f"A:ON:{id_alarma}" if nuevo_estado else f"A:OFF:{id_alarma}"
            enviar(cmd)
...
def tarea_conectar():
    ...
    try:
        nuevo = serial.Serial(puerto, 115200, timeout=1)
        time.sleep(1.0) 
        ...
        registrar_log(f"Conexión exitosa establecida en {puerto} a 115200 bps")
...
def agregar_nueva_alarma_formulario():
    if len(estado.alarmas) >= estado.MAX_ALARMAS_HARDWARE:
        messagebox.showerror("Límite Alcanzado", f"Se ha alcanzado el límite máximo del sistema ({estado.MAX_ALARMAS_HARDWARE} alarmas).\n\nElimina una alarma existente si deseas agregar otra nueva.")
        return
    ...
    id_tomados = [a["id"] for a in estado.alarmas]
    id_libre = 0
    for ranura in range(estado.MAX_ALARMAS_HARDWARE):
        if ranura not in id_tomados:
            id_libre = ranura
            break

    nueva_al = {"id": id_libre, "hora": hora, "min": minuto, "periodo": periodo, "duracion": duracion, "dias": mascara, "activa": True}
    
    estado.alarmas.append(nueva_al)
    actualizar_lista_visual()

    letra_periodo = "P" if periodo == "PM" else "A"
    comando_alarma = f"A:{id_libre}:{int(hora)}:{int(minuto)}:{letra_periodo}:{duracion}:{mascara}"
    
    if estado.arduino and estado.arduino.is_open:
        enviar(comando_alarma)
        registrar_log(f"Alarma guardada en ranura física {id_libre+1}: {hora}:{minuto} {periodo}")
    else:
        registrar_log(f"Alarma guardada de forma local temporal en ID {id_libre+1}")
        messagebox.showwarning("Guardado Local", "Alarma guardada en la interfaz.\n\nNota: El Arduino está desconectado. Sincroniza al conectar.")
    ...
    while estado.escuchando and estado.arduino:
        try:
            if estado.arduino.in_waiting > 0:
                ...
                elif linea.startswith("ALERTA:ALARMA_ACTIVA") or "ALARMA_ENCENDIDA" in linea:
                    registrar_log("RX 🚨 ALERTA: Alarma en curso en el Hardware.")
...
def actualizar_reloj_desde_arduino(linea):
    try:
        ...
        if len(partes) >= 48 and not estado.alarmas:
            idx_datos = 6
            for i in range(7):
                habilitada = True if partes[idx_datos+5] == "1" else False
                if habilitada:
                    alarm_struct = {
                        "id": i,
                        "hora": f"{int(partes[idx_datos]):02d}",
                        "min": f"{int(partes[idx_datos+1]):02d}",
                        "periodo": "PM" if partes[idx_datos+2] == "P" else "AM",
                        "duracion": partes[idx_datos+3],
                        "dias": int(partes[idx_datos+4]),
                        "activa": True
                    }
                    estado.alarmas.append(alarm_struct)
                idx_datos += 6
            actualizar_lista_visual()
    except:
        pass
...
registrar_log("Terminal iniciada. Sistema listo.")
app.mainloop()
...
# ==========================================
# VENTANA EMERGENTE DE ALARMA (Controlada solo por Arduino)
# ==========================================
def mostrar_alerta_emergente():
    """ Esta ventana solo se abrirá cuando el Arduino envíe el comando de sirena """
    ...
    ctk.CTkLabel(frame_interior, text="🔔 ACTIVACIÓN DE ALARMA PROGRAMADA", font=("Consolas", 22, "bold"), text_color="#33FF5F").pack(pady=(20, 5))
    
    ctk.CTkLabel(frame_interior, text="El reloj (Arduino) ha activado la alarma programada.\n\nEl reloj/Arduino está reproduciendo\nla alarma en sus paneles físicos.", font=("Consolas", 14)).pack(pady=10)
...
def boton_eliminar_alarma(id_alarma):
    ...
    registrar_log(f"Alarma de interfaz ID {id_alarma} eliminada.")
...
def agregar_nueva_alarma_formulario():
    ...
    if estado.arduino and estado.arduino.is_open:
        ...
        registrar_log(f"Nueva Alarma Guardada y enviada al hardware: {hora}:{minuto} {periodo}")
    else:
        registrar_log(f"Alarma guardada localmente (Pendiente de envío)")
        # Ajuste de mensaje UX para indicar que sin hardware no habrá alarma
        messagebox.showwarning("Guardado Local", "Alarma guardada en la interfaz.\n\nNota: El Arduino está desconectado. Debes conectarlo para enviar la configuración, de lo contrario la alarma no sonará.")

def hilo_lector_serial():
    """ Escucha estrictamente los comandos enviados desde el Hardware """
    while estado.escuchando and estado.arduino:
        try:
            if estado.arduino.in_waiting > 0:
                ...
                # El único gatillo para la ventana emergente ahora es el Arduino
                elif linea.startswith("ALERTA:ALARMA_ACTIVA"):
                    registrar_log("RX 🚨 ALERTA: Sirena en curso en el Hardware.")
                    app.after(0, mostrar_alerta_emergente)
...
buscar_puertos() 
actualizar_lista_visual() 
registrar_log("Terminal iniciada. Sistema sujeto al hardware maestro.")
app.mainloop()